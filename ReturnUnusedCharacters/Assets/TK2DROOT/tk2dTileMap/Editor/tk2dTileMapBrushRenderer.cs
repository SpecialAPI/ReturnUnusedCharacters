	...
	public class BrushRenderer
	{
		...
		void BuildMeshForBrush(tk2dTileMapEditorBrush brush, BrushDictData dictData, int tilesPerRow)
		{
			...
			foreach (var spriteDef in spriteCollection.spriteDefinitions) {
				if (spriteDef.Valid)
					spriteBounds = Vector3.Max(spriteBounds, spriteDef.untrimmedBoundsDataExtents);
			}
			...
			else
			{
				...
				foreach (var tile in brush.tiles)
				{
					...
					if (brush.overrideWithSpriteBounds) {
						tileOrigin.x += spriteBounds.x * 0.5f - sprite.untrimmedBoundsDataCenter.x;
						tileOrigin.y += spriteBounds.y * 0.5f - sprite.untrimmedBoundsDataCenter.y;
					}
		...
		public Rect TileSizePixels
		{
			get 
			{
				...
				foreach (var spriteDef in spriteCollection.spriteDefinitions) {
					if (spriteDef.Valid)
						tileSize = Vector3.Max(tileSize, spriteDef.untrimmedBoundsDataExtents);
				}
