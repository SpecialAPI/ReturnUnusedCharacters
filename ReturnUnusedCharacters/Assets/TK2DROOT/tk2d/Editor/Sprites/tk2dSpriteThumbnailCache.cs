...
public static class tk2dSpriteThumbnailCache
{
	...
	{
		return new Vector2(def.untrimmedBoundsDataExtents.x / def.texelSize.x, def.untrimmedBoundsDataExtents.y / def.texelSize.y);
	}
	...
	public static void DrawSpriteTextureInRect(Rect rect, tk2dSpriteDefinition def, Color tint) {
		...
		if (Event.current.type == EventType.Repaint) {
			float sw = def.untrimmedBoundsDataExtents.x;
			float sh = def.untrimmedBoundsDataExtents.y;
	...
	{
		Init();
		Vector2 pixelSize = new Vector3( rect.width / def.untrimmedBoundsDataExtents.x, rect.height / def.untrimmedBoundsDataExtents.y);
		...
		if (Event.current.type == EventType.Repaint && visible)
		{
			if (def.materialInst != null) {
				...
				Vector3 t = def.untrimmedBoundsDataExtents * 0.5f - def.untrimmedBoundsDataCenter;
				float tq = def.untrimmedBoundsDataExtents.y;
