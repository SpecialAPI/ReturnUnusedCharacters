...
public static class tk2dSpriteGeomGen
{
	...
	public static void SetClippedSpriteGeom( Vector3[] pos, Vector2[] uv, int offset, out Vector3 boundsCenter, out Vector3 boundsExtents, tk2dSpriteDefinition spriteDef, Vector3 scale, Vector2 clipBottomLeft, Vector2 clipTopRight, float colliderOffsetZ, float colliderExtentZ )
	{
		...
		if (spriteDef.positions.Length == 4)
		{
			...
			Vector3 untrimmedMin = spriteDef.untrimmedBoundsDataCenter - spriteDef.untrimmedBoundsDataExtents * 0.5f;
			Vector3 untrimmedMax = spriteDef.untrimmedBoundsDataCenter + spriteDef.untrimmedBoundsDataExtents * 0.5f;
			...
			Vector3 trimmedBounds = spriteDef.boundsDataExtents;
			Vector3 trimmedOrigin = spriteDef.boundsDataCenter - trimmedBounds * 0.5f;
	...
	{
		int numTilesX = (int)Mathf.Ceil( (dimensions.x * spriteDef.texelSize.x) / spriteDef.untrimmedBoundsDataExtents.x );
		int numTilesY = (int)Mathf.Ceil( (dimensions.y * spriteDef.texelSize.y) / spriteDef.untrimmedBoundsDataExtents.y );
	...
	public static void SetTiledSpriteGeom(Vector3[] pos, Vector2[] uv, int offset, out Vector3 boundsCenter, out Vector3 boundsExtents, tk2dSpriteDefinition spriteDef, Vector3 scale, Vector2 dimensions, tk2dBaseSprite.Anchor anchor, float colliderOffsetZ, float colliderExtentZ)
	{
		...
		int vert = 0;
		Vector3 bounds = Vector3.Scale(  spriteDef.untrimmedBoundsDataExtents, scale );
